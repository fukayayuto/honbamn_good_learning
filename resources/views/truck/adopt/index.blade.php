<!DOCTYPE html>
<html lang="ja">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>
      ご採用実績｜トラックドライバー教育のクラウド型eラーニング【グッドラーニング!】
    </title>
    <meta
      name="title"
      content="ご採用実績｜トラックドライバー教育のクラウド型eラーニング【グッドラーニング!】"
    />
    <meta
      name="description"
      content="グッドラーニング！をご採用頂いたお客様をご紹介いたします。"
    />
    <meta
      name="keywords"
      content="乗務員,教育,研修,Eラーニング,指導,国交省,トラック,運送業,グッドラーニング"
    />
    <meta
      property="og:title"
      content="ご採用実績｜トラックドライバー教育のクラウド型eラーニング【グッドラーニング!】"
    />
    <meta property="og:type" content="article" />
    <meta
      property="og:url"
      content="https://promote.good-learning.jp/truck/faq/"
    />
    <meta
      property="og:image"
      content="https://promote.good-learning.jp/truck/common/img/shared/og_image.jpg"
    />
    <meta
      property="og:site_name"
      content="トラックドライバー教育のクラウド型eラーニング【グッドラーニング!】"
    />
    <meta
      property="og:description"
      content="グッドラーニング！をご採用頂いたお客様をご紹介いたします。"
    />
    <link rel="canonical" href="https://promote.good-learning.jp/truck/faq/" />
    <link rel="stylesheet" href="https://use.typekit.net/hcg7pyj.css" />
    <link rel="preconnect" href="https://fonts.googleapis.com" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
    <link
      href="https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@400;700"
      rel="stylesheet"
      media="print"
      onload="this.media=’all'"
    /> 
    <link href="{{ asset('/truck/css/default.css') }}" rel="stylesheet" type="text/css" />
    <link href="{{ asset('/truck/css/layout.css') }}" rel="stylesheet" type="text/css" />
    <link href=".{{ asset('/truck/css/content.css') }}" rel="stylesheet" type="text/css" />
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/2.2.0/jquery.min.js"></script>
    <?php @include($_SERVER['DOCUMENT_ROOT']."/truck/common/inc/head_before.inc")?>
  </head>

  <body>
    <?php @include($_SERVER['DOCUMENT_ROOT']."/truck/common/inc/body_after.inc")?>
    <div id="wrapper">
      <!-- header// -->
      <header>
        <div id="header">
          <p id="headLogo">
            <a href="/truck/"
              ><img 
                src="{{ asset('/truck/img/shared/head_logo.svg') }}"
                alt="グッドラーニング"
                width="262"
                height="50"
                loading="lazy"
            /></a>
          </p>
          <p id="headTxt">
            <span>トラックドライバー教育</span>のクラウド型eラーニング
          </p>
        </div>
        <div id="global">
          <div id="navTrigger"><span>&nbsp;</span></div>
          <nav>
            <ul>
              <li><a href="/good_learning/truck/">HOME</a></li>
              <li><a href="/good_learning/truck/price/">料金について</a></li>
              <li><a href="/good_learning/truck/flow/">ご利用の流れ</a></li>
              <li class="stay"><span>ご採用実績</span></li>
              <li><a href="/good_learning/truck/faq/">FAQ</a></li>
              <li><a href="/good_learning/truck/contact/">お問い合わせ</a></li>
            </ul>
          </nav>
        </div>
      </header>
      <!-- //header -->
      <!-- main// -->
      <main>
        <div id="pageTit">
          <h1>ご採用実績</h1>
          <div id="breadCrumbs">
            <ol itemscope="" itemtype="https://schema.org/BreadcrumbList">
              <li
                itemprop="itemListElement"
                itemscope=""
                itemtype="https://schema.org/ListItem"
              >
                <a href="/truck/" itemprop="item"
                  ><span itemprop="name">HOME</span></a
                ><meta itemprop="position" content="1" />
              </li>
              <li>ご採用実績</li>
            </ol>
          </div>
        </div>
        <p id="pageTxt">
          グッドラーニング！をご採用いただいた企業様をご紹介いたします。北は北海道から南は九州・沖縄まで、多くの企業様・ドライバー様にお使いいただき、安心・安全の一助となっております。
        </p>
        <div class="inner">
          <div id="adopt">
            <ul>
              <li>G's Logicom株式会社</li>
              <li>株式会社九建運送</li>
              <li>協和興運株式会社</li>
              <li>有限会社成田商事</li>
              <li>有限会社美珠商事</li>
              <li>ロジカ株式会社</li>
              <li>株式会社アルティス</li>
              <li>浜松委託運送株式会社</li>
              <li>株式会社大全</li>
              <li>三多摩トランスポート株式会社</li>
              <li>メッセンジャーエクスプレス株式会社</li>
              <li>竜王運輸有限会社</li>
              <li>株式会社橋本運送</li>
              <li>石川運送株式会社</li>
              <li>株式会社第一名誠</li>
              <li>株式会社武蔵野運輸</li>
              <li>株式会社浦安清運</li>
              <li>有限会社石田運送</li>
              <li>有限会社駒井運輸</li>
              <li>株式会社中田運送</li>
              <li>林工業所株式会社</li>
              <li>大牟田運送株式会社</li>
              <li>一般社団法人 京都府トラック協会</li>
              <li>信濃物流株式会社</li>
              <li>株式会社生興運送</li>
              <li>株式会社生興運送 本社</li>
              <li>ＯＷＬ株式会社</li>
              <li>株式会社ヒラバヤシ環境サービス</li>
              <li>株式会社廣川運送</li>
              <li>株式会社カーワークス</li>
              <li>ティーロジエクスプレス株式会社</li>
              <li>三光運輸株式会社</li>
              <li>三原物流株式会社</li>
              <li>九州栄孝エキスプレス株式会社</li>
              <li>株式会社京都物流運輸</li>
              <li>東群エクスプレス株式会社</li>
              <li>郡東エクスプレス株式会社</li>
              <li>株式会社よし笛流通</li>
              <li>株式会社三共運輸</li>
              <li>株式会社丸和運輸機関</li>
              <li>江差運送株式会社</li>
              <li>石川商運株式会社</li>
              <li>株式会社ＡＬＬ　ＪＡＰＡＮ</li>
              <li>株式会社萩原・運輸</li>
              <li>株式会社CarryService</li>
              <li>株式会社イカイトランスポート</li>
              <li>中日本システム輸送株式会社</li>
              <li>株式会社前河通商</li>
              <li>尾上運送株式会社</li>
              <li>ＨＰＬ株式会社</li>
              <li>有限会社あか音物流</li>
              <li>株式会社オレンヂ</li>
              <li>株式会社ザイマックストラベルデザイン</li>
              <li>中讃通運株式会社</li>
              <li>有限会社藤崎運輸</li>
              <li>西福運送株式会社</li>
              <li>静神運輸株式会社</li>
              <li>株式会社エスペラント</li>
              <li>株式会社丸野</li>
              <li>栃木ダイキュー運輸株式会社</li>
              <li>有限会社小菅運輸</li>
              <li>城東運輸倉庫株式会社</li>
              <li>有限会社拓伸運輸</li>
              <li>有限会社菱和産業</li>
              <li>株式会社ホンダパーツ西南</li>
              <li>有限会社竹内運送</li>
              <li>株式会社北辰運輸</li>
              <li>有限会社扇運送店</li>
              <li>有限会社ハッスイトランスポート</li>
              <li>有限会社イムラ運送</li>
              <li>江口運送有限会社</li>
              <li>株式会社福島北桑運輸</li>
              <li>長洲運送株式会社</li>
              <li>株式会社ヤマキエクスプレス</li>
              <li>株式会社高久商事</li>
              <li>株式会社スグル貨物運輸</li>
              <li>株式会社いわれ</li>
              <li>水間急配株式会社</li>
              <li>大豊物流株式会社</li>
              <li>道南綜合土建株式会社</li>
              <li>丸十運送株式会社</li>
              <li>大津中央運送株式会社</li>
              <li>有限会社日協</li>
              <li>郡山物流有限会社</li>
              <li>郡山物流有限会社</li>
              <li>有限会社井上商事</li>
              <li>株式会社酒井運送</li>
              <li>よつ葉カーゴネット株式会社</li>
              <li>山崎運輸株式会社</li>
              <li>清水運送有限会社</li>
              <li>株式会社出版産業</li>
              <li>株式会社グリーンハートエクスプレス</li>
              <li>一般社団法人大阪府トラック協会東北支部</li>
              <li>株式会社岩崎運送</li>
              <li>株式会社Ｖ．Ａ．Ｐ．</li>
              <li>株式会社古賀組</li>
              <li>日東運送株式会社</li>
              <li>北総通運株式会社</li>
              <li>株式会社駒来商事</li>
              <li>拓大運輸有限会社</li>
              <li>有限会社早川運輸</li>
              <li>有限会社サクセス</li>
              <li>静岡通運株式会社</li>
              <li>九州ネクスト株式会社</li>
              <li>有限会社松葉重機</li>
              <li>ペンタ・ロジスティクス株式会社</li>
              <li>名古屋千里運輸株式会社</li>
              <li>株式会社坂本建運</li>
              <li>大竹配送有限会社</li>
              <li>黒田運輸株式会社</li>
              <li>一般社団法人三重県トラック協会</li>
              <li>東洋商事株式会社</li>
              <li>三和運送有限会社</li>
              <li>丸八急配社有限会社</li>
              <li>株式会社アライアンス・コーポレーション</li>
              <li>大翔トランスポート株式会社</li>
              <li>大同商運株式会社</li>
              <li>有限会社豊和運輸倉庫</li>
              <li>駿河荷役運輸株式会社</li>
              <li>ナガシマ物流サービス株式会社</li>
              <li>櫻井商事運輸株式会社</li>
              <li>宝通商株式会社</li>
              <li>有限会社大丸運輸</li>
              <li>有限会社干潟運送</li>
              <li>有限会社髙田運送</li>
              <li>東京共同ロジテム株式会社</li>
              <li>幸和運輸株式会社</li>
              <li>株式会社丸伝運送グループ</li>
              <li>関西低温株式会社</li>
              <li>朝日運輸株式会社</li>
              <li>株式会社高木運輸</li>
              <li>株式会社双蹊</li>
              <li>清里トランスポート株式会社</li>
              <li>森トク運輸株式会社</li>
              <li>有限会社西野商店</li>
              <li>株式会社丸神商事運輸</li>
            </ul>
          </div>
        </div>
        <div id="contactBox">
          <div id="contactIn">
            <p>お問い合わせ・受講体験は<br />お電話又はフォームで</p>
            <a href="tel:0368801064" class="tel">03-6880-1064</a>
            <a href="/truck/contact/" class="btn"
              >お問い合わせ・受講体験はこちら</a
            >
          </div>
        </div>
      </main>
      <!-- //main -->
      <!-- footer// -->
      <footer>
        <div class="inner">
          <div class="footLogo">
            <img 
              src="{{ asset('/truck/img/shared/foot_logo.svg') }}"
              alt="グッドラーニング"
              width=""
              height=""
              loading="lazy"
            />
          </div>
          <ul class="footNav">
            <li><a href="/good_learning/truck/">ホーム</a></li>
            <li><a href="/good_learning/truck/price/">料金について</a></li>
            <li><a href="/good_learning/truck/flow/">ご利用の流れ</a></li>
            <li><a href="/good_learning/truck/adopt/">ご採用実績</a></li>
            <li><a href="/good_learning/truck/faq/">FAQ</a></li>
            <li><a href="/good_learning/truck/contact/">お問い合わせ</a></li>
          </ul>
        </div>
        <div id="copy">
          <div class="inner">
            <ul class="footSub">
              <li>
                <a href="https://cab-station.com/privacy.html" target="_blank"
                  >プライバシーポリシー</a
                >
              </li>
              <li>
                <a href="https://cab-station.com/company.html" target="_blank"
                  >会社概要</a
                >
              </li>
            </ul>
            <p>© 2021 Cab Station Co., Ltd.</p>
          </div>
        </div>
      </footer>
      <!-- //footer -->
		<div id="float">
		<p><span>お問い合わせ・受講体験はお電話で</span></p>
		<a href="tel:0368801064">03-6880-1064<span>今すぐ電話する</span></a>
		</div>
    </div>
    <!-- js -->
    <script>
      (function (d) {
        var config = {
            kitId: "bli4hjx",
            scriptTimeout: 3000,
            async: true,
          },
          h = d.documentElement,
          t = setTimeout(function () {
            h.className =
              h.className.replace(/\bwf-loading\b/g, "") + " wf-inactive";
          }, config.scriptTimeout),
          tk = d.createElement("script"),
          f = false,
          s = d.getElementsByTagName("script")[0],
          a;
        h.className += " wf-loading";
        tk.src = "https://use.typekit.net/" + config.kitId + ".js";
        tk.async = true;
        tk.onload = tk.onreadystatechange = function () {
          a = this.readyState;
          if (f || (a && a != "complete" && a != "loaded")) return;
          f = true;
          clearTimeout(t);
          try {
            Typekit.load(config);
          } catch (e) {}
        };
        s.parentNode.insertBefore(tk, s);
      })(document);
    </script> 
    <script src="{{ asset('/truck/js/common.css') }}"></script>
    <?php @include($_SERVER['DOCUMENT_ROOT']."/truck/common/inc/js_after.inc")?>
  </body>
</html>
