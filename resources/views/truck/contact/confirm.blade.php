...
$inquirer = h($data['inquirer']);
$mail_address = h($data['mail_address']);
$tel = h($data['tel']);
$pref_address = h($data['pref_name'] . " " . $data['address']);
$date = h($data['year'] . $data['month']);

if ($data["human"] != '') {
	$human = h('およそ' . $data['human'] . '人');
} else {
	$human = "--";
}
  ...
  <body>
    ...
    <div id="wrapper">
      ...
      <main>
        ...
        <div class="inner">
          <div id="contact">
            <form method="post" action="{{route('truck_contact_thanks')}}">
              @csrf
              <!-- <div id="error">
						〇〇が入力されていません
					</div> -->
